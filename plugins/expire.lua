local function pre_process(msg)
	...
	if expiretime then    
		...
		if msg.text:match('/') then
			return send_large_msg(get_receiver(msg), '📆تاریخ انقضای گروه به پایان رسید.📆')
		else
			return
		end
	end
	...
	if tonumber(timetoexpire) == 2 then
		...
		send_large_msg(get_receiver(msg), '📆2روز تا پایان تاریخ انقضای گروه باقی مانده است . 📆\n😊نسبت به تمدید اقدام کنید . 🏵 ')
		redis:hset('expires2',msg.to.id,'5')
	end
	if tonumber(timetoexpire) == 3 then
		if redis:hget('expires3',msg.to.id) then return msg end
		send_large_msg(get_receiver(msg), '📆3روز تا پایان تاریخ انقضای گروه باقی مانده است . 📆\n😊نسبت به تمدید اقدام کنید . 🏵 ')
...
function run(msg, matches)
	...
	if matches[1]:lower() == 'مدت زمان' then
		local expiretime = redis:hget ('expiretime', get_receiver(msg))
