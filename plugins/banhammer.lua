...
local function pre_process(msg)
  ...
  if msg.action and msg.action.type then
    ...
    if action == 'chat_add_user_link' then
      ...
      savelog(msg.to.id, name.." ["..msg.from.id.."] 🛡کاربر بن و اخراج شد . 😐 ")-- Save to logs
...
local support_id = msg.from.id
 if matches[1]:lower() == 'id' and msg.to.type == "chat" or msg.to.type == "user" then
  ...
  if matches[1]:lower() == "���� ��" then -- Ban list !
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == '��' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
		...
		send_large_msg(receiver, 'کاربر ['..matches[2]..'] به لیست بن اضافه شد')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'ban',
  ...
  if matches[1]:lower() == '��� ��' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	return 'کاربر '..user_id..' از لیست بن حذف شد'
      else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unban',
...
if matches[1]:lower() == '�����' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "شما نمیتوانید مدیران/ادمین ها/ صاحب گروه را اخراج کنید"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
